...
Model.deleteOneWithRedis = async function deleteOneWithRedis(conditions, options, callback) {
  await redis.del(conditions.id || conditions._id);
  return this.deleteOne(conditions, options, callback);
};
 ...
 *
 *     // Find one adventure whose `country` is 'Croatia', otherwise `null`
 *     await Adventure.findOneWithRedis({ country: 'Croatia' }).exec();
 *
 *     // using callback
 *     Adventure.findOneWithRedis({ country: 'Croatia' }, function (err, adventure) {});
 *
 *     // select only the adventures name and length
 *     await Adventure.findOneWithRedis({ country: 'Croatia' }, 'name length').exec();
 *
 * @param {Object} [conditions]
 * @param {Object|String|Array<String>} [projection] optional fields to return, see [`Query.prototype.select()`](#query_Query-select)
 * @param {Object} [options] optional see [`Query.prototype.setOptions()`](http://mongoosejs.com/docs/api.html#query_Query-setOptions)
...
Model.findOneWithRedis = async function findOne(conditions, projection, options, callback) {
  const redisData = await redis.get(conditions.id || conditions._id);
  return redisData ? JSON.parse(redisData) : this.findOne(conditions, projection, options, callback).lean().then(async x => {
    if (x) {
      await redis.set(x.id || x._id, JSON.stringify(x));
      return x;
    }
    return x || this.create(conditions, options);
  });
};

Model.findOneAndUpdateWithRedis = function findOneAndUpdateWithRedis(conditions, update, options = {}, callback) {
  _checkContext(this, 'updateOne');
  options = Object.assign({ upsert: true, new: true }, options);
  return this.findOneAndUpdate(conditions, update, options, callback).lean().then(async x => {
    if (x) await redis.set(x.id || x._id, JSON.stringify(x));
    return x;
  });
};
...
Model.create = async function create(doc, options, callback) {

  const self = this;

  function xd() {

    _checkContext(self, 'create');
    let args;
    let cb;
    const discriminatorKey = self.schema.options.discriminatorKey;
    ...
    return self.db.base._promiseOrCallback(cb, cb => {
      cb = self.$wrapCallback(cb);
      ...
      args.forEach(doc => {
        toExecute.push(callback => {
          const Model = self.discriminators && doc[discriminatorKey] != null ?
            self.discriminators[doc[discriminatorKey]] || getDiscriminatorByValue(self.discriminators, doc[discriminatorKey]) :
            self;
          if (Model == null) {
            throw new MongooseError(`Discriminator "${doc[discriminatorKey]}" not ` +
              `found for model "${self.modelName}"`);
          }
          let toSave = doc;
      ...
      const _done = (error, res) => {
        ...
        if (doc instanceof Array) {
          ...
          cb.apply(self, [null].concat(savedDocs));
        }
      };
  ...
  await redis.set(res.id || res._id, JSON.stringify(res));
